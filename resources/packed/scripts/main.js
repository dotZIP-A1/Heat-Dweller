...
class GameEngine {
  ...
  loadConfig() {
    return new Promise((resolve) => {
      fetch('resources/packed/scripts/xml/config.xml')
        .then(response => response.text())
        .then(xml => {
          const parser = new DOMParser();
          this.config = parser.parseFromString(xml, 'application/xml');
          this.initGameState();
          resolve();
        })
        .catch(err => {
  ...
  initGameState() {
    ...
    // Load assets
    const spriteSheetPath = getVal('assets > spriteSheet', 'resources/packed/sprites/characters/character1.png');
    const backgroundPath = getVal('assets > background', 'resources/packed/sprites/enviornement/grassbackground1.png');
    
    this.spriteSheet.image.src = spriteSheetPath;
    this.spriteSheet.image.onload = () => { this.spriteSheet.loaded = true; };
    
    this.swordSprites.image.src = 'resources/packed/sprites/attack/sword/';
    this.swordSprites.image.onload = () => { this.swordSprites.loaded = true; };
  ...
  initGameStateDefault() {
    ...
    this.spriteSheet.image.src = 'resources/packed/sprites/characters/character1.png';
    this.spriteSheet.image.onload = () => { this.spriteSheet.loaded = true; };
    
    this.backgroundImage.image.src = 'resources/packed/sprites/enviornement/grassbackground1.png';
    this.backgroundImage.image.onload = () => { this.backgroundImage.loaded = true; };
  }
  ...
  updateAttack(delta) {
    ...
    if (this.player.attacking) {
      ...
      if (this.player.animationTime >= this.player.attackDuration) {
  ...
  triggerAttack(direction) {
    if (this.player.attackCooldown <= 0) {
      ...
      this.player.animationTime = 0;
      this.player.lastAttackDirection = direction;
      this.player.attackCooldown = 1 / this.stats.attackSpeed;
    }
  }
  ...
  drawSpriteFrame(frameIndex, drawX, drawY, flip = false) {
    const frame = this.spriteSheet.frames[frameIndex];
    if (flip) {
      this.ctx.save();
      this.ctx.translate(drawX + frame.w, drawY);
      this.ctx.scale(-1, 1);
      this.ctx.drawImage(
  ...
  drawGrid() {
    ...
    for (let x = 0; x <= this.world.width; x += this.world.tileSize) {
      const drawX = x - this.camera.x;
      if (drawX >= -this.world.tileSize && drawX <= this.camera.width + this.world.tileSize) {
        this.ctx.beginPath();
        this.ctx.moveTo(drawX, -this.camera.y);
        this.ctx.lineTo(drawX, this.world.height - this.camera.y);
        this.ctx.stroke();
      }
    }
    
    for (let y = 0; y <= this.world.height; y += this.world.tileSize) {
      const drawY = y - this.camera.y;
      if (drawY >= -this.world.tileSize && drawY <= this.camera.height + this.world.tileSize) {
        this.ctx.beginPath();
        this.ctx.moveTo(-this.camera.x, drawY);
        this.ctx.lineTo(this.world.width - this.camera.x, drawY);
        this.ctx.stroke();
      }
    }
  }
  
  getAttackRotation(direction) {
    switch(direction) {
      case 'right':
        return 0;
      case 'down':
  ...
  drawSwordAttack(centerX, centerY, direction, attackProgress) {
    if (!this.swordSprites.loaded) return;
    
    const rotation = this.getAttackRotation(direction);
    const swordWidth = 64;
  ...
  drawPlayer() {
    ...
    const centerX = drawX + this.player.width / 2;
    const centerY = drawY + this.player.height / 2;
    
    // Draw idle Matthew
    if (this.spriteSheet.loaded) {
      this.drawSpriteFrame(2, drawX, drawY, false);
      this.drawSpriteFrame(headFrameIndex, drawX, drawY, false);
    } else {
    ...
    if (this.player.attacking) {
      this.drawSwordAttack(centerX, centerY, this.player.lastAttackDirection, this.player.animationTime / this.player.attackDuration);
    }
  }
  ...
  drawHUD() {
    ...
    this.ctx.fillText(`Status: ${this.player.attacking ? 'Attacking' : 'Ready'}`, 16, 24);
    this.ctx.fillText(`Direction: ${this.player.lastAttackDirection}`, 16, 44);
    this.ctx.fillText('Use arrow keys to attack in different directions.', 16, 64);
  }
  ...
  setInput(event, value) {
    ...
    // Trigger attack on key down
    if (value) {
      switch (event.code) {
        case 'ArrowLeft':
        case 'KeyA':
          this.triggerAttack('left');
          break;
        case 'ArrowRight':
        case 'KeyD':
          this.triggerAttack('right');
          break;
        case 'ArrowUp':
        case 'KeyW':
          this.triggerAttack('up');
          break;
        case 'ArrowDown':
        case 'KeyS':
          this.triggerAttack('down');
          break;
      }
    }
  }
  ...
  async init() {
    ...
    window.addEventListener('keydown', (event) => {
      if (event.code === 'Enter' && this.gameState === 'title') {
        this.startGame();
        return;
      }
    ...
    const titleEl = document.getElementById('titleScreen');
    if (titleEl) {
      titleEl.addEventListener('click', () => this.startGame());
    }
